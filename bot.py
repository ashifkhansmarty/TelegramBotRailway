...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "stats":
        if user_id in ADMINS:
            ...
            await query.message.reply_text(f"📊 Stats Panel\n\nTotal Users: {total_users}\nTotal Searches: {total_searches}")
        else:
    ...
    if query.data == "mycredits":
        ...
        if credits > 0:
            ...
            await query.message.reply_text(
                "❌ You have 0 credits left.\nPlease contact Admin to refill your credits."
            )
# ------------------------------------------
...
async def handle_mobile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await msg.edit_text("⚠ API timeout. Try again later.")
        return
    except:
    ...
    if isinstance(result, dict) and result.get("message") == "No records found":
        await msg.edit_text("❌ No records found for this number.")
        return
...
"""
    ...
    await msg.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard) if keyboard else None)
# ------------------------------------------
...
async def navigation_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'results' not in context.user_data:
        await query.message.edit_text("❌ No results to navigate.")
        return
    ...
    context.user_data['index'] = max(0, min(context.user_data['index'], len(context.user_data['results'])-1))
...
def main():
    ...
    # Buttons
    app.add_handler(CallbackQueryHandler(button_handler, pattern="^(lookup|stats|mycredits)$"))
    app.add_handler(CallbackQueryHandler(navigation_handler, pattern="^(next|prev)$"))
